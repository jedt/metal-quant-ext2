    ...
    )
{
    ...
    @autoreleasepool {
        ...
        dispatch_sync(serialQueue, ^(){
            ...
            torch::mps::commit();
            NSLog(@"commandBuffer.status: %@", commandBuffer.status == MTLCommandBufferStatusCompleted ? @"Completed" : @"Not Completed");
            // Check for errors (remains the same)
            if (commandBuffer.status == MTLCommandBufferStatusError) {
                NSLog(@"Command buffer error: %@", commandBuffer.error);
                return;
            }

            // Convert half to float after kernel execution
            uint16_t *rawScales = (uint16_t *)scalesBuffer.contents;
            uint16_t *rawOffsets = (uint16_t *)offsetsBuffer.contents;

            // Directly write to PyTorch tensor data
            ...
            NSLog(@"blockwise_quant completed successfully");
        });
